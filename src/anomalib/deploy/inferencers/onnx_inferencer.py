"""ONNX Inferencer implementation."""
...
if find_spec("onnxruntime") is not None:
    ...
    logger.warning("ONNXRuntime is not installed. Please install ONNXRuntime to use ONNXInferencer.")
...
class ONNXInferencer(Inferencer):
    ...
    Examples:
        Assume that we have an ONNX IR model and metadata files in the following structure:
        ...
        We could then create ``ONNXInferencer`` as follows:
    ...
    def __init__(
        ...
        if self.device.capitalize() == "CPU":
            self.execution_provider = ["CPUExecutionProvider"]
        elif self.device.capitalize() == "GPU":
            self.execution_provider = ["CUDAExecutionProvider", "CPUExecutionProvider"]
        else:
    ...
    def load_model(self, path: str | Path | tuple[bytes, bytes]) -> "InferenceSession":
        """Load the ONNX model.
        ...
        """
        model = ort.InferenceSession(path, providers=self.execution_provider)
        return model
    ...
    def forward(self, image: np.ndarray) -> np.ndarray:
        ...
        """
        outputs = self.model.run(output_names=["output"], input_feed={"input": image})
        return outputs
